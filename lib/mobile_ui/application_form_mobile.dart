...
import '../web_ui/application_forms/documents_upload.dart';
import '../web_ui/widgets/show_instructions_dialog.dart';
...
class DynamicFormBuilderM extends StatefulWidget {
  const DynamicFormBuilderM({super.key});

  @override
  State<DynamicFormBuilderM> createState() => _DynamicFormBuilderState();
}

class _DynamicFormBuilderState extends State<DynamicFormBuilderM> {
                  ...
                  Padding(
                    ...
                    child: Row(
                      ...
                      children: [
                        ...
                        Expanded(child: Text("Imdaad Talimi Application Form",style: TextStyle(fontSize: 18,fontWeight: FontWeight.bold),)),
                      ],
                    ),
                  ),
                  Padding(
              ...
              LayoutBuilder(
                builder: (context, constraints) {
                  ...
                  for (int i = 0; i < fieldsList.length; i++) {
                    ...
                    fieldRows.add(Column(children: rowChildren));
  ...
  Widget buildMiniRepeatableGroup(
    ...
    // If no rows yet, enqueue one entry
    if (entries.isEmpty) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      children: [
            ...
            children: entries.asMap().entries.map((entryPair) {
              ...
              return Container(
                ...
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.stretch,
                  children: [
                    // ─── For each field in this entry: show label + widget ───────
                    ...fields.map<Widget>((f) {
                      final String fKey = f['key'] as String;
                      final String fLabel = f['label'] as String? ?? '';
                      ...
                      if (fType == 'dropdown') {
                        ...
                        // 2) If it’s “familyMembers”, filter out duplicates
                      ...
                      // ─── TEXT (or “fetch-its”, “radio”, etc.) ──────────────────
                      return constants.buildField(
                        fLabel,                          // label (positional)
                        value as RxString,               // controller
                        this,                            // parent widget
                        validatorKey: f['validator'] as String?,
                        validator: (v) => validateField(fLabel, v ?? '', validatorKey: f['validator'] as String?),
        ...
        Align(
          ...
          child: ElevatedButton.icon(
            ...
            onPressed: () {
              // Only add a new entry if all existing ones are valid
              final allValid = entries.every((e) =>
                  fields.every((f) {
                    final val = e[f['key']];
                    if (f['type'] == 'dropdown') {
...
class SectionStepper extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final bool isMobile = screenWidth < 600; // you can adjust this breakpoint

    // Color logic for the progress bar
    Color getProgressColor() {
      if (completionPercent >= 100) return Colors.green;
      if (completionPercent >= 70) return Colors.lightGreen;
      if (completionPercent >= 40) return Colors.orange;
      if (completionPercent > 0) return Colors.blueGrey;
      return Colors.redAccent;
    }

    // -----------------------------------------
    // 1️⃣ If mobile, show a simple "Step X of Y" + bar
    // -----------------------------------------
    if (isMobile) {
      // activeIndex is zero‐based; for human read, add 1
      final int displayIndex = activeIndex + 1;
          ...
          children: [
            ...
            Row(
              ...
              children: [
                ...
                // You could also show the current step title (truncated)
                Expanded(
                  child: Padding(
            ...
            // Linear progress for overall completion percent
            ClipRRect(
    ...
    // -----------------------------------------
    // 2️⃣ Desktop/tablet mode: original "wrapped chips" + percent
    // -----------------------------------------
    return Padding(
      padding: const EdgeInsets.symmetric(horizontal: 30.0, vertical: 8.0),
      child: Column(
        children: [
          // ❌ You can reduce wrap spacing if too “wide”
          Wrap(
            spacing: 12,
            runSpacing: 8,
            children: sections.asMap().entries.map((entry) {
              final int index = entry.key;
              final SectionStep section = entry.value;
              final bool isActive = index == activeIndex;

              // Decide colors for each “chip”
          ...
          // ❌ The overall horizontal progress bar is still shown
          Row(
            children: [
              Expanded(
                child: TweenAnimationBuilder<double>(
                  tween: Tween<double>(
                      begin: 0, end: completionPercent / 100),
