import 'package:get/get.dart';
import '../controller/state_management/state_manager.dart';
import '../model/family_data2.dart';

class DropdownValues {

  static GlobalStateController get _ctr => Get.find();


  static Map<String, List<Map<String, dynamic>>> dropdownOptions2 = {
    "assetsOptions": [
      {"name": "Gas Stove"},
      {"name": "TV"},
      {"name": "Radio"},
      {"name": "Telephone/Mobile"},
      {"name": "Animal Cart"},
      {"name": "Bicycle"},
      {"name": "Computer/Laptop"},
      {"name": "Motorbike"},
      {"name": "Refrigerator"},
      {"name": "Washing Machine"},
      {"name": "Refrigerator"},
      {"name": "Car"},
      {"name": "Truck"},
    ],
    "houseTitleOptions": [
      {"id": 0, "name": "Ownership - On Own Plot"},
      {"id": 1, "name": "Ownership - In Society"},
      {"id": 2, "name": "Ownership - In Slum Area"},
      {"id": 3, "name": "Pagdi - In Chawl"},
      {"id": 4, "name": "Pagdi - In Slum Area"},
      {"id": 5, "name": "Heavy Deposit - In Society"},
      {"id": 6, "name": "Heavy Deposit - In Slum Area"},
      {"id": 7, "name": "Rental - In Society"},
      {"id": 8, "name": "Rental - In Chawl"},
      {"id": 9, "name": "Rental - In Slum Area"},
      {"id": 10, "name": "Staying Without Rent"}
    ],
    "houseAreaOptions": [
      {"id": 0, "name": "Smaller than 200 sqft"},
      {"id": 1, "name": "200-300 sqft"},
      {"id": 2, "name": "300-400 sqft"},
      {"id": 3, "name": "400-500 sqft"},
      {"id": 4, "name": "500-600 sqft"},
      {"id": 5, "name": "600 sqft & above"},
      {"id": 6, "name": "Family up to 2 - Below Carpet 150 - 1RK"},
      {"id": 7, "name": "Family up to 4 - Below Carpet 300"},
      {"id": 8, "name": "Family up to 6 - Below Carpet 450"}
    ],
    "neighborhoodOptions": [
      {"id": 0, "name": "Mumineen Mohalla"},
      {"id": 1, "name": "Muslimeen Area - Liveable"},
      {"id": 2, "name": "Muslimeen Area - Non Liveable"},
      {"id": 3, "name": "Cosmopolitan Area - Liveable"},
      {"id": 4, "name": "Cosmopolitan Area - Non Liveable"},
      {"id": 5, "name": "Others - Non Liveable"}
    ],
    "drinkingWaterOptions": [
      {"id": 0, "name": "Handpump - Drinkable Water"},
      {"id": 1, "name": "Handpump - Non-Drinkable Water"},
      {"id": 2, "name": "Boring - Drinkable Water"},
      {"id": 3, "name": "Boring - Non-Drinkable Water"},
      {"id": 4, "name": "Municipal Water - Filtered / RO Water"},
      {"id": 5, "name": "Municipal Water - Non-Filtered Water"}
    ],
    "sanitationBathroomOptions": [
      {"id": 0, "name": "Combined Bathroom/Toilet - Inside House"},
      {"id": 1, "name": "Separate Bathroom/Toilet - Inside House"},
      {"id": 2, "name": "Outside in Chawl"},
      {"id": 3, "name": "Outside in Government Sauchalay"},
      {"id": 4, "name": "Outside in Jungle/Open"}
    ],
    "incomeTypeOptions": [
      {"id": 0, "name": "Business"},
      {"id": 1, "name": "Professional"},
      {"id": 2, "name": "Salary"},
      {"id": 3, "name": "Investment"},
      {"id": 4, "name": "Property Rent"},
      {"id": 5, "name": "Home Industry"}
    ],
    "causeOptions": [
      {
        "id": 0,
        "name": "Accident",
      },
      {
        "id": 1,
        "name": "Natural Cause",
      },
      {
        "id": 2,
        "name": "Disease",
      }
    ],
    "namazDailyOptions": [
      {"id": 0, "name": "All five Faraz"},
      {"id": 1, "name": "Some Faraz"},
      {"id": 2, "name": "Doesn’t pray"},
      {"id": 3, "name": "Doesn’t Know"}
    ],
    "shaadiOptions": [
      {"id": 0, "name": "Married"},
      {"id": 1, "name": "Single"},
      {"id": 2, "name": "Widow"},
      {"id": 3, "name": "Divorcee"},
      {"id": 4, "name": "Engaged"}
    ],
    "yesNoOption": [
      {"id": 0, "name": "Yes"},
      {"id": 1, "name": "No"},
    ],
    "asharaAttendanceOptions": [
      {"id": 0, "name": "All 9 days"},
      {"id": 1, "name": "Few Days"},
      {"id": 2, "name": "Only Ashura"},
      {"id": 3, "name": "Not attended"}
    ],
    "fmbThaliOptions": [
      {"id": 0, "name": "Yes daily"},
      {"id": 1, "name": "Not daily but occasionally"},
      {"id": 2, "name": "Not receiving"}
    ],
    "fmbQuantityOptions": [
      {"id": 0, "name": "Yes for 1 time"},
      {"id": 1, "name": "Not enough for 1 time"},
      {"id": 2, "name": "Yes for 2 times"}
    ],
    "propertyOptions": [
      {"id": 0, "name": "Rent"},
      {"id": 1, "name": "Owned"},
      {"id": 2, "name": "Goodwill"}
    ],
    "khidmatOptions": [
      {"id": 0, "name": "Burhani Guards International"},
      {"id": 1, "name": "Tolobatul Kulliyat ul Mumenoon"},
      {"id": 2, "name": "Shabab ul Eid-e-Zahabi"},
      {"id": 3, "name": "Dawat Offices"},
      {"id": 4, "name": "12 Umoor"},
      {"id": 5, "name": "None"}
    ],
    "khidmatIntentOptions": [
      {"id": -1, "name": "None"},
      {"id": 0, "name": "Burhani Guards International"},
      {"id": 1, "name": "Tolobatul Kulliyat ul Mumenoon"},
      {"id": 2, "name": "Shabab ul Eid-e-Zahabi"},
      {"id": 3, "name": "Dawat Offices"},
      {"id": 4, "name": "12 Umoor"},
      {"id": 5, "name": "None"}
    ],
    "semesterOptions": [
      {"id": 0, "name": "Playgroup"},
      {"id": 1, "name": "Nursery"},
      {"id": 2, "name": "Kindergarten"},
      {"id": 3, "name": "Class 1"},
      {"id": 4, "name": "Class 2"},
      {"id": 5, "name": "Class 3"},
      {"id": 6, "name": "Class 4"},
      {"id": 7, "name": "Class 5"},
      {"id": 8, "name": "Class 6"},
      {"id": 9, "name": "Class 7"},
      {"id": 10, "name": "Class 8"},
      {"id": 11, "name": "Class 9"},
      {"id": 12, "name": "Class 10"},
      {"id": 13, "name": "Class 11"},
      {"id": 14, "name": "Class 12"},
      {"id": 15, "name": "Semester 1"},
      {"id": 16, "name": "Semester 2"},
      {"id": 17, "name": "Semester 3"},
      {"id": 18, "name": "Semester 4"},
      {"id": 19, "name": "Semester 6"},
      {"id": 20, "name": "Semester 7"},
      {"id": 21, "name": "Semester 8"},
      {"id": 22, "name": "Semester 10"},
      {"id": 23, "name": "Al Jamea Tus Saifiyah"},
      {"id": 24, "name": "Hifz (Tehfeez)"},
    ],
    "sportsOptions": [
      {"name": "Football"},
      {"name": "Cricket"},
      {"name": "Basketball"},
      {"name": "Tennis"},
      {"name": "Badminton"},
      {"name": "Volleyball"},
      {"name": "Table Tennis"},
      {"name": "Hockey"},
      {"name": "Swimming"},
      {"name": "Gym / Cardio"},
      {"name": "Walking"},
      {"name": "Gymnastics"},
      {"name": "Others"},
    ],
    // "familyMembers": [
    //   {"id": 20377228, "name": "Aboaali"},
    //   {"id": 30445122, "name": "Tasneem"},
    //   {"id": 30445123, "name": "Qutbuddinb"}
    // ],};

     "familyMembers": _ctr.familyMembers
         .map((fm) => {
       'id': fm.itsNumber,
       'name': fm.fullName,
     }).toList(),
    };

}
