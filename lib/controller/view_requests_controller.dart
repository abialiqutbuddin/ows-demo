...
import '../api/api.dart';
import 'admin/view_req_forms.dart';
...
class _ReqFormTableState extends State<ReqFormTable> {
  ...
  Future<void> fetchFilteredRequests() async {
    ...
    try {
      ...
      await reqFormController.fetchRequests(
          widget.mohalla, widget.org, widget.ITS, widget.role);
  ...
  void _showStatusChangeDialog(RequestFormModel request, List<int> featureIds) {
    if (!featureIds.contains(7)) {
      Get.snackbar(
          "Access Denied", "You do not have permission to change the status.",
        ...
        builder: (context, setStateDialog) {
          ...
          return Column(
            ...
            children: [
              ...
              ElevatedButton(
                onPressed: () async {
                  bool success = await Api.updateRequestStatus(
                      request.reqId!, selectedStatus);
                  if (success) {
